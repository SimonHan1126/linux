...
static int simcom_wwan_bind(struct usbnet *dev, struct usb_interface *intf)
{
    ...
    if (dev->udev->descriptor.bcdDevice == cpu_to_le16(0x318)  && intf->cur_altsetting->desc.bInterfaceNumber == 5)
    {
        printk("sim7600 device\n");
...
static int simcom_wwan_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	int ret = usbnet_suspend(intf, message);
	return ret;
}
